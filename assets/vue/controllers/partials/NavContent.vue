  ...
  <div class="flex flex-col md:flex-row justify-between w-full">
    <div class="flex flex-col md:flex-row md:items-center text-sm md:flex-grow">
      <div v-for="item in navigations" :key="item.name" class="mb-4 md:mb-0 md:mr-4">
        <a v-if="item.show" :href="item.href"
          :class="[item.name === currentItem.name ? 'bg-gray-50 dark:bg-gray-950 text-blue-700 dark:text-gray-300' : 'text-gray-700 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 hover:text-blue-700 dark:hover:text-gray-300', 'rounded-md px-3 py-2 text-sm font-medium']"
