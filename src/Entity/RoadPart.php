...
class RoadPart
{
  ...
  #[ORM\OneToOne(mappedBy: 'roadPart')]
  ...
  #[ORM\Column(nullable: true)]
  private ?\DateTimeImmutable $staggedAt = null;

  #[ORM\Column(nullable: true)]
  private ?int $pickingDurationSeconds = null;
  ...
  public function getStaggedAt(): ?\DateTimeImmutable
  {
    return $this->staggedAt;
  }

  private function setStaggedAt(?\DateTimeImmutable $staggedAt): static
  {
    $this->staggedAt = $staggedAt;

    return $this;
  }

  public function getPickingDurationSeconds(): ?int
  {
    return $this->pickingDurationSeconds;
  }

  private function setPickingDurationSeconds(?int $pickingDurationSeconds): static
  {
    $this->pickingDurationSeconds = $pickingDurationSeconds;
  ...
  {
    $this->setStagged(true);
    $this->setStaggedAt(new \DateTimeImmutable());

    if ($this->getPickingStartedAt()) {
      $duration = $this->getStaggedAt()->getTimestamp() - $this->getPickingStartedAt()->getTimestamp();
